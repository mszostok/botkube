...
func (b *SocketSlack) send(event socketSlackMessage, req string, resp interactive.Message) error {
	...
	if len(markdown) >= slackMaxMessageSize {
		...
		// The message sections were already sent, we only need to
		// take care of sending inputs it there are any.
...
func (b *SocketSlack) resolveBlockActionCommand(callback slack.InteractionCallback) string {
	...
	case "plain_text_input":
		command = fmt.Sprintf("%s %s --filter=%q", b.BotName(), act.BlockID, command)
	}
