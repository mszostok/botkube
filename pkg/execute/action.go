...
func (e *ActionExecutor) List(ctx context.Context, cmdCtx CommandContext) (api.Message, error) {
...
func (e *ActionExecutor) Enable(ctx context.Context, cmdCtx CommandContext) (api.Message, error) {
	const enabled = true
	cmdVerb, cmdRes := parseCmdVerb(cmdCtx.Args)
	...
	if err := e.cfgManager.PersistActionEnabled(ctx, actionName, enabled); err != nil {
		return api.Message{}, fmt.Errorf("while setting action %q to %t: %w", actionName, enabled, err)
	}
...
func (e *ActionExecutor) Disable(ctx context.Context, cmdCtx CommandContext) (api.Message, error) {
	const enabled = false
	cmdVerb, cmdRes := parseCmdVerb(cmdCtx.Args)
